...
// ====== Bangun URL domain + direktori 'absensi-qr/' ======
$httpsOn = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') || ($_SERVER['SERVER_PORT'] ?? '') == 443;
$protocol = $httpsOn ? 'https://' : 'http://';
$host     = $_SERVER['HTTP_HOST'] ?? 'localhost';

// Cari apakah ada "/absensi-qr" di request URI
$requestUri = $_SERVER['REQUEST_URI'] ?? '/';
$pos = strpos($requestUri, '/absensi-qr/');
if ($pos !== false) {
    $dirPath = substr($requestUri, 0, $pos + strlen('/absensi-qr/'));
} else {
    $dirPath = '/absensi-qr/';
}
...
<p> HARAP GUNAKAN NOMOR WHATSAPP BISNIS UNTUK MENGINDARI BLOKIR/SPAM </p>
            ...
            <p class="text-muted small mb-0">
              *Halaman ini otomatis menggunakan <strong>domain + direktori <code>/absensi-qr/</code></strong>.<br>
